...
We also need to skip the first line of mci_amyloid.csv since it is not the header for dataframe. Also to follow the format of `demo_info_df`, the variable `study_id` is renamed as `id`. There are missing values in dataset, it might means those participants didn't show up at that time. The size of the dataframe is 487 x 6. Also, we could pivot the dataset longer, it was named as `amyloid_pivoted_df`. 
